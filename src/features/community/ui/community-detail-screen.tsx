...
export function CommunityDetailScreen({ initialPost, postId = initialPost?.id ?? '' }: { postId?: string; initialPost?: CommunityPost }) {
  ...
  const copyRoute = async () => { if (!post.route) return; try { await navigator.clipboard.writeText(post.route.shareLink); toast.success('Troadie 링크를 복사했어요.'); } catch { toast.error('링크를 복사하지 못했어요.'); } };
