...
import { chatKeys, useChatRooms } from '@/features/chat/api/chat-queries';
...
export function CommunityDetailScreen({ postId }: { postId: string }) {
  ...
  const queryClient = useQueryClient(); const rooms = useChatRooms();
  ...
  const room = rooms.data?.find((item) => item.companionPostId === id);
  const accept = (commentId: number) => { if (!room) return; void chatService.accept(room.roomId, commentId).then(() => { toast.success('동행 참여를 수락했어요.'); void queryClient.invalidateQueries({ queryKey: chatKeys.rooms() }); void detail.refetch(); }).catch(fail); };
  ...
  return <AppShell showBottomNavigation={false} header={<PageHeader title='게시글' backHref='/community' action={userId === post.authorId ? <button type='button' aria-label='게시글 삭제' onClick={() => setDeleteOpen(true)} className='grid size-9 place-items-center text-red-600'><Trash2 size={18} /></button> : null} />}><ContentContainer className='space-y-7'>
    ...
    {post.type === 'COMPANION' && room ? <Link href={`/chat/${room.roomId}`} className='inline-flex rounded-xl bg-violet-600 px-4 py-2 text-sm font-semibold text-white'>채팅방 입장</Link> : null}
