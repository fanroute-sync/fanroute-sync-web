...
export function CommunityWriteScreen() {
  ...
  const [type, setType] = useState<CommunityPostType>('INFO');
  const createPost = useCreatePost();
  const publish = (post: CommunityPost) => createPost.mutate(post, { onSuccess: () => { toast.success('게시글을 등록했어요.'); router.push('/community'); } });
  ...
  return <AppShell showBottomNavigation={false} header={<PageHeader title='글쓰기' backHref='/community' />}><ContentContainer className='space-y-6'><Tabs items={tabs} value={type} onValueChange={setType} ariaLabel='게시글 타입' />{type === 'INFO' ? <InfoPostForm onSubmit={submitInfo} loading={createPost.isPending} /> : null}{type === 'REFERENCE_ROUTE' ? <RoutePostForm onSubmit={submitRoute} loading={createPost.isPending} /> : null}{type === 'COMPANION' ? <CompanionPostForm onSubmit={submitCompanion} loading={createPost.isPending} /> : null}</ContentContainer></AppShell>;
}
