...
import { logoutSession } from '@/features/onboarding/api/auth-session-service';
import { toast } from 'sonner';

export function MyPageScreen({ data, showFixtureActivity = true }: { data: MyPageData; showFixtureActivity?: boolean }) {
  const router = useRouter(); const queryClient = useQueryClient(); const [logoutOpen, setLogoutOpen] = useState(false);
  const logout = async () => {
    try {
      await logoutSession();
    } catch {
      toast.error('서버 로그아웃을 완료하지 못했어요. 다시 로그인할 때 세션을 확인해주세요.');
    } finally {
      queryClient.clear();
  ...
  return <AppShell header={<Header title='마이페이지' />}><ContentContainer className='space-y-7'>
    <section className='flex items-center gap-4'><span className='grid size-16 place-items-center rounded-full bg-violet-100 text-violet-600'><UserRound aria-hidden='true' size={30} /></span><div className='min-w-0 flex-1'><h2 className='text-xl font-bold'>{data.profile.nickname}</h2><p className='text-sm text-gray-500'>Troadie 여행자</p></div><Link href='/my/profile' aria-label='프로필 수정' className='grid size-10 place-items-center rounded-full bg-gray-100'><Pencil aria-hidden='true' size={18} /></Link></section>
    {showFixtureActivity ? <><section><h2 className='mb-3 text-lg font-bold'>내 활동</h2><Card className='grid grid-cols-3 divide-x p-0 shadow-none'><Link href='/my/activity?tab=POST' className='py-4 text-center'><FileText aria-hidden='true' className='mx-auto text-violet-600' size={19} /><strong className='mt-2 block'>{data.activityCounts.posts}개</strong><span className='text-xs text-gray-500'>게시글</span></Link><Link href='/my/activity?tab=LIKE' className='py-4 text-center'><Heart aria-hidden='true' className='mx-auto text-violet-600' size={19} /><strong className='mt-2 block'>{data.activityCounts.likes}개</strong><span className='text-xs text-gray-500'>좋아요</span></Link><Link href='/my/activity?tab=COMMENT' className='py-4 text-center'><MessageCircle aria-hidden='true' className='mx-auto text-violet-600' size={19} /><strong className='mt-2 block'>{data.activityCounts.comments}개</strong><span className='text-xs text-gray-500'>댓글</span></Link></Card></section><Link href='/my/ai-usage' className='flex items-center gap-4 rounded-2xl bg-violet-700 p-5 text-white'><Bot aria-hidden='true' size={28} /><span className='flex-1'><strong className='block'>AI 루트 사용 현황</strong><span className='mt-1 block text-sm text-violet-100'>{data.aiUsage.used} / {data.aiUsage.limit}회 사용</span></span><ChevronRight aria-hidden='true' /></Link></> : null}
